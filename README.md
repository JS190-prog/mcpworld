...
> ℹ️ **CAD·Photoshop MCP**는 별도 공개 배포처가 없는 커스텀/자작 서버입니다.
> 사용하려면 `config.json`의 해당 `install` 저장소 주소(`YOUR_GITHUB`)를 본인
> 저장소나 호환 MCP로 교체하세요. Blender·HWP는 공개 출처, Office는 `uvx`라
> 그대로 동작합니다.
...
| CAD(업로드) | 18003 | 8013 | https://YOUR_DOMAIN/cmcp-upload/mcp |
